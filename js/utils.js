...
function ShowBurgerMenu() {
  ...
  window.addEventListener("resize", function () {
    if (screen.width > 600) {
      ...
      if (
        this.window.getComputedStyle(burgerIcon).getPropertyValue("display") ===
          "none" &&
        this.window.getComputedStyle(closeIcon).getPropertyValue("display") ===
          "none"
      ) {
        burgerIcon.style.display = "block";
      }
...
function CreateProjectCards(projectsData) {
  ...
  for (let i = 0; i < projectsData.length; i++) {
    ...
    projectDuration.innerHTML =
      duration + " | " + projectsData[i].meta.technologies;
    ...
    projectCard.insertAdjacentHTML(
      "beforeend",
...
function FAQAccrodion() {
  ...
  document.querySelectorAll(".q-container").forEach((question) => {
    question.addEventListener("click", function () {
      ...
      if (
        window.getComputedStyle(plusButton).getPropertyValue("display") ===
        "block"
      ) {
